...
_LEADING_ARTICLES = re.compile(r"^(the|a|an|my|that|this|nearest|closest|farthest|furthest|biggest|largest)(\s+|$)", re.I)
...
def wants_largest(text: str) -> bool:
    """True if the user asked for the biggest/largest instance, which picks
    among multiple detections by bounding-box area rather than score or depth."""
    return bool(re.search(r"\b(biggest|largest)\b", text, re.I))
